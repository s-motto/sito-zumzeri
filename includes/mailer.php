...
// ── CONFERMA DEFINITIVA CAMERA ────────────────────────────────────────────────

function invia_email_conferma_definitiva_camera(array $dati): bool
{
  $mail = new PHPMailer(true);
  ...
  try {
    ...
    $mail->isHTML(true);
    $mail->Subject = 'Prenotazione confermata — Zum Zeri [' . $dati['codice'] . ']';
    $mail->Body    = email_body_conferma_definitiva_camera($dati);
    $mail->AltBody = email_testo_conferma_definitiva_camera($dati);

    $mail->send();
    return true;
  } catch (Exception $e) {
    error_log('Errore email conferma definitiva camera: ' . $mail->ErrorInfo);
    return false;
  }
}

function email_body_conferma_definitiva_camera(array $d): string
{
  $check_in  = date('d/m/Y', strtotime($d['check_in']));
  $check_out = date('d/m/Y', strtotime($d['check_out']));
  $notti     = (new DateTime($d['check_in']))->diff(new DateTime($d['check_out']))->days;
  ...
  return "
    ...
    <body style='font-family: Georgia, serif; color: #3d2e10; max-width: 600px; margin: 0 auto; padding: 0;'>
      ...
      <div style='padding: 40px 32px;'>
        <h2 style='font-size: 22px; font-weight: 400; margin: 0 0 8px;'>Ciao {$d['nome']},</h2>
        <p style='color: #666; line-height: 1.7; margin: 0 0 32px;'>
          Siamo lieti di comunicarti che la tua prenotazione è stata <strong style='color: #2e7d32;'>confermata</strong>. Non vediamo l'ora di accoglierti!
        </p>

        <div style='background: #e8f5e9; border-radius: 4px; padding: 24px; margin-bottom: 32px; border: 1px solid #a5d6a7;'>
          <p style='font-size: 11px; letter-spacing: 2px; text-transform: uppercase; color: #aaa; margin: 0 0 8px;'>Prenotazione confermata</p>
          <p style='font-size: 36px; letter-spacing: 6px; color: #2e7d32; margin: 0 0 8px;'>{$d['codice']}</p>
          <p style='font-size: 12px; color: #aaa; margin: 0;'>Conserva questo codice per qualsiasi comunicazione con noi</p>
        </div>
        ...
        <p style='color: #666; font-size: 14px; line-height: 1.7;'>
          Per qualsiasi informazione puoi rispondere a questa email o contattarci al numero che trovi sul sito. A presto!
        </p>
      </div>
...
function email_testo_conferma_definitiva_camera(array $d): string
{
  $check_in  = date('d/m/Y', strtotime($d['check_in']));
  $check_out = date('d/m/Y', strtotime($d['check_out']));
  return "
...
siamo lieti di comunicarti che la tua prenotazione è stata CONFERMATA. Non vediamo l'ora di accoglierti!
...
Per qualsiasi informazione contattaci tramite il sito. A presto!

Zum Zeri · Passo dei Due Santi · 54029 Zeri (MS)
    ";
}

// ── CONFERMA DEFINITIVA RISTORANTE ────────────────────────────────────────────

function invia_email_conferma_definitiva_ristorante(array $dati): bool
{
  $mail = new PHPMailer(true);
  ...
  try {
    ...
    $mail->isHTML(true);
    $mail->Subject = 'Prenotazione ristorante confermata — Zum Zeri [' . $dati['codice'] . ']';
    $mail->Body    = email_body_conferma_definitiva_ristorante($dati);
    $mail->AltBody = email_testo_conferma_definitiva_ristorante($dati);

    $mail->send();
    return true;
  } catch (Exception $e) {
    error_log('Errore email conferma definitiva ristorante: ' . $mail->ErrorInfo);
    return false;
  }
}

function email_body_conferma_definitiva_ristorante(array $d): string
{
  return "
    <!DOCTYPE html>
    <html lang='it'>
    ...
    <body style='font-family: Georgia, serif; color: #3d2e10; max-width: 600px; margin: 0 auto;'>
      ...
      <div style='padding: 40px 32px;'>
        <h2 style='font-size: 22px; font-weight: 400; margin: 0 0 8px;'>Ciao {$d['nome']},</h2>
        <p style='color: #666; line-height: 1.7; margin: 0 0 32px;'>
          Siamo lieti di comunicarti che la tua prenotazione al ristorante è stata <strong style='color: #2e7d32;'>confermata</strong>. Non vediamo l'ora di accoglierti!
        </p>
...
function email_testo_conferma_definitiva_ristorante(array $d): string
{
  return "
Ciao {$d['nome']},

siamo lieti di comunicarti che la tua prenotazione al ristorante è stata CONFERMATA. Non vediamo l'ora di accoglierti!
